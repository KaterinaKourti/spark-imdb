...
# ----------------------------------------------------------------------------------------
# Group By 'genre'
gb_genre = final_cubed.filter(
    final_cubed.genre.isNotNull() & final_cubed.gender.isNull()
)
...
# ----------------------------------------------------------------------------------------
# Group By 'gender'
gb_gender = final_cubed.filter(
    final_cubed.gender.isNotNull() & final_cubed.genre.isNull()
)
...
gb_genre_gender = final_cubed.filter(
    final_cubed.gender.isNotNull() & final_cubed.genre.isNotNull()
)
...
female_ratings = gb_genre_gender.withColumnRenamed(
    "avg_rating", "female_avg_rating"
).filter(gb_genre_gender.gender == "F")
male_ratings = gb_genre_gender.withColumnRenamed(
    "avg_rating", "male_avg_rating"
).filter(gb_genre_gender.gender == "M")
joined = male_ratings.join(female_ratings, ["genre"])
joined.createOrReplaceTempView("GenreRatingsPerGender")

data = spark.sql(
    "SELECT genre FROM GenreRatingsPerGender WHERE female_avg_rating > male_avg_rating"
)
data.show()
...
rating_per_genre = (
    final_joined.where(col("genre") == "Adventure")
    .groupBy(col("rating"))
    .count()
...
ax.legend(
    wedges, labels, title="Ratings", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1)
)
